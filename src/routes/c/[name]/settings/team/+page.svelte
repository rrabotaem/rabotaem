<script lang="ts">
  ...
  async function addModerator() {
    ...
    try {
      const searchName = formData.newModerator.trim().replace(/^@+/, '')
      
      const searchResponse = await fetch(`https://${env.PUBLIC_INSTANCE_URL}/api/v3/search?q=${searchName}&type_=Users&limit=1`, {
        headers: {
          "accept": "*/*",
      ...
      const userId = searchRes.users[0].person.id
      ...
      if (!addModRes || !addModRes.moderators) {
        throw new Error('Ошибка при добавлении модератора')
      }
      ...
      toast({
        content: err?.message || 'Пользователь не найден',
        type: 'error'
      })
    }
