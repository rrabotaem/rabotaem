<script lang="ts">
  ...
  import { ban, isAdmin, modals } from '$lib/components/lemmy/moderation/moderation.js'
  ...
  import type { View } from '$lib/settings.js'

  // Определяем тип для view с явным указанием всех возможных значений
  type ViewType = 'All' | 'Local' | 'Subscribed' | 'Community' | 'ModQueue' | 'all' | 'comments' | 'posts';
  ...
  $: {
    if (data.items?.length) {
      const comments = data.items.filter(isCommentView);
      if (comments.length > 0) {
        console.debug('First comment structure:', comments[0]);
      }
    }
  }

  function isValidView(view: string): view is View {
    return ['All', 'Local', 'Subscribed', 'Community', 'ModQueue'].includes(view);
  }

  // Приводим тип к View или undefined
  $: currentView = data.type as View;
</script>
...
<div class="flex flex-col gap-4 max-w-[640px] mx-auto w-full">
  {#if !inline}
    <div class="">
      <EntityHeader
        class="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800"
      ...
      <!-- Меню администратора -->
      {#if $profile?.user && isAdmin($profile.user) && data.person_view.person.id !== $profile.user.local_user_view.person.id}
        <div class="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-4 mt-4">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold flex items-center gap-2">
              <Icon src={ShieldCheck} size="20" mini />
              Администрирование
            </h3>
            <Menu placement="bottom-start">
              <Button size="square-md" slot="target" color="danger-subtle">
              ...
              >
                ...
                />
                {data.person_view.person.banned ? 'Разбанить пользователя' : 'Забанить пользователя'}
              </MenuButton>
              <MenuButton
                color="danger-subtle"
      ...
      <div class="flex flex-col gap-4">
        {#each data.items as item}
          ...
          {:else if !isCommentView(item) && (data.type === 'all' || data.type === 'posts')}
            <div class="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 px-4">
              <div class="py-2">
                <Post post={item} />
              </div>
            </div>
...
<BanModal
  ...
  bind:user={$modals.banning.user}
  bind:banned={$modals.banning.banned}
  bind:community={$modals.banning.community}
/>
