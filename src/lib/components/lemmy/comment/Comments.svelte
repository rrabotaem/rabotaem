<script lang="ts">
  ...
  export let autoReplyTo: number | undefined = undefined
...
>
  {#each nodes as node (node.comment_view.comment.id)}
    <Comment
      ...
      autoReply={autoReplyTo === node.comment_view.comment.id}
    >
      {#if node.children?.length > 0}
        <svelte:self {post} bind:nodes={node.children} isParent={false} {autoReplyTo} />
      {/if}
      {#if node.comment_view.counts.child_count > 0 && node.children.length == 0}
        {#await fetchChildren(node)}
          <div class="w-full my-2 h-8">
            <Button
              loading={true}
              disabled={true}
              rounding="pill"
